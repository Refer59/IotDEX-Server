...
export const signUpConfirmation = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
export const protectRoute = (onlyForRoles: string[]) => catchAsync(async (req: Request, res: Response, next: NextFunction, dataForCallback?: string[]) => {
    const authorizationString: string = req.headers.authorization
    let token
    ...
    const decoded: any = await promisify<string, string>(jwt.verify)(token, process.env.JWT_SECRET)
...
export const getAuthDataByAuth = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    if (req.body.user)
        res.status(200).json({
            status: 'Sucess',
            user: req.body.user
...
export const checkUserCreationToken = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    if (!req.params.token)
...
export const checkUserResetToken = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    if (!req.params.token)
        return next(new AppError('Propocione un token de creacion de cuenta', 400))
